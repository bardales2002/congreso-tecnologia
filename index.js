...
function generarDiploma({ nombre, actividad, fecha }, cb) {
  ...
  const rojo = '#b60000', rojoOsc = '#8f0000', grisClaro = '#e9edf3', grisTxt = '#1f2937';
...
db.query('SELECT 1', (err) => {
  if (err) { console.error('❌ MySQL ping falló:', err); process.exit(1); }
...
app.get('/debug/smtp', async (req, res) => {
  try {
    await new Promise((resolve, reject) => transporter.verify(e => e ? reject(e) : resolve()));
    const info = await transporter.sendMail({
      from: `"Congreso UMG" <${process.env.SMTP_USER}>`,
      to: process.env.SMTP_USER,
      subject: 'Prueba SMTP (Render)',
      text: 'Hola, mensaje de prueba desde Render.'
    });
    ...
    const enviarCorreoConQR = () => {
      ...
      QRCode.toDataURL(textoQR, { errorCorrectionLevel: 'H' }, (errQR, dataURL) => {
        ...
        const htmlImg = attachment
          ...
          : (dataURL ? `<img src="${dataURL}" width="160" height="160" style="display:block;border:0;">`
        ...
        const mailOptions = {
          ...
          html: `<p>Hola <strong>${nombre}</strong>:</p>
                 ...
                 ${htmlImg}`,
          attachments: attachment ? [{ filename: 'qr.png', content: attachment, cid: 'qrimg' }] : []
        };

        transporter.sendMail(mailOptions, (e, info) => {
          if (e) console.error('✉️ Error enviando QR:', e.message);
          else console.log('✉️ QR enviado:', info && info.response);
        });

        res.json({ success: true, msg: 'Inscripción exitosa' });
      });
    };
