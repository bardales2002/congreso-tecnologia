...
function generarDiploma({ nombre, actividad, fecha }, cb) {
  ...
  doc.fillColor('#fff').font('Helvetica-Bold').fontSize(18).text('Universidad Mariano Gálvez de Guatemala', 40, 24, { width: W - 80, align: 'center' }).moveDown(0.2).font('Helvetica').fontSize(14).text('Facultad de Ingeniería en Sistemas', { width: W - 80, align: 'center' });
  const cx = W / 2;
  const cy = bandH + 55;
  const logoSize = 110;
  ...
  doc.fillColor(grisTxt).font('Helvetica-Bold').fontSize(26).text('Diploma de Participación', 40, topAfterLogo, { width: W - 80, align: 'center' });
  const decoW = 140;
  ...
  doc.moveDown(1.2);
  doc.font('Helvetica').fontSize(14).fillColor('#111').text(`Por su destacada participación en ${actividad}.`, { width: W - 120, align: 'center' });
  doc.moveDown(1.5);
  doc.font('Helvetica-Oblique').fontSize(12).fillColor('#111').text(`Guastatoya, ${fecha}`, { width: W - 80, align: 'right' });
  const yFirmas = H - 150;
  const sep = 200;
  const x1 = cx - sep - 40;
  const x2 = cx + 40;
  doc.save().lineWidth(1.2).strokeColor(grisClaro).moveTo(x1, yFirmas).lineTo(x1 + 180, yFirmas).stroke().moveTo(x2, yFirmas).lineTo(x2 + 180, yFirmas).stroke().restore();
  doc.font('Helvetica-Bold').fontSize(11).fillColor('#111').text('Decano(a)', x1, yFirmas + 6, { width: 180, align: 'center' }).text('Coordinador(a)', x2, yFirmas + 6, { width: 180, align: 'center' });
...
function getResultados(anio, res) {
  ...
  db.query(sql, [anio], (err, rows) => {
    ...
    rows.forEach(r => {
      ...
      mapa[r.act_id].ganadores.push({ puesto: r.puesto, nombre: r.ganador, descripcion: r.descripcion, foto: r.foto_url });
    });
